    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < N + Q; i++) {
            ...
            if (decision.equals("insert")) {
                tree.insert(number);
            } else if (decision.equals("ask")) {
                BNode<Integer> node = tree.find(number);
                BNode<Integer> root = tree.getRoot();

                int cnt = findGreaterThan(node, root);
                System.out.println(cnt);
            }
        }
    }
    private static int findGreaterThan(BNode<Integer> node, BNode<Integer> root) {
        if (root == null) {
            return 0;
        }
        int count = 0;
        ...
        count += findGreaterThan(node, root.left);
        count += findGreaterThan(node, root.right);
