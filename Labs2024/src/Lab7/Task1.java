    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n + q; i++) {
            ...
            if (parts[0].equals("root")) {
                ...
                int childIndex = Integer.parseInt(parts[2]);
                parentNode = nodes.get(parentIndex);
                childNode = (SLLTree.SLLNode<Integer>) tree.addChild(parentNode, childIndex);
                nodes.put(childIndex, childNode);
            } else if (parts[0].equals("ask")) {
                int nodeIndex = Integer.parseInt(parts[1]);
                SLLTree.SLLNode<Integer> node = nodes.get(nodeIndex);
...
class SLLTree<E> implements Tree<E> {
    ...
    public static class SLLNode<P> implements Tree.Node<P> {
    ...
    public class NodeIterable implements Iterable<Tree.Node<E>>{

        Tree.Node<E> start;
        public NodeIterable(Tree.Node<E> start) {
            this.start = start;
        }

        @Override
        public Iterator<Tree.Node<E>> iterator() {
            return new NodeIterator(start);
        }
    }

    public class NodeIterator implements Iterator<Tree.Node<E>> {
        Tree.Node<E> current;

        public NodeIterator(Tree.Node<E> node) {
            current = node;
        }
