    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < N + Q; i++) {
            ...
            if (what.equals("insert")) {
                ...
                int elem = sc.nextInt();
                BNode<Integer> node = tree.find(elem);
                BNode<Integer> root = tree.getRoot();

                System.out.println(findSmallerElements(root, node));

            }
        }
    }

    private static int findSmallerElements(BNode<Integer> root, BNode<Integer> node) {
        if (root == null) {
            return 0;
        }
        int count = 0;
        ...
        count += findSmallerElements(root.left, node);
        count += findSmallerElements(root.right, node);
