...
public class Main {
    public static void main(String[] args) {
        ...
        int count = checkSum(startVertex, wantedSum, graph, startVertex);

        System.out.println(count);
    }

    private static int checkSum(int currentVertex, int wantedSum, AdjacencyListGraph<Integer> graph, int currentSum) {
        // If the sum exceeds the wantedSum, stop exploring further
        if (currentSum > wantedSum) {
            return 0;
        }

        // If the sum is exactly the wantedSum, we found a valid path
        if (currentSum == wantedSum) {
            return 1;
        }
        ...
        for (int neighbor : graph.getNeighbors(currentVertex)) {
            ...
            count += checkSum(neighbor, wantedSum, graph, currentSum + neighbor);
        }
