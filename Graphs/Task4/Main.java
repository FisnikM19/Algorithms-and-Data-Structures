...
public class Main {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < list.size(); i++) {
            if (!set.contains(list.get(i))) {
                count++;
                checkCities(list.get(i), set, graph);
            }
        }
    ...
    private static void checkCities(String s, Set<String> set, AdjacencyListGraph<String> graph) {
        if (set.contains(s)) {
            return;
        }
        ...
        for (String city: graph.getNeighbors(s)) {
            checkCities(city, set, graph);
        }
    }
