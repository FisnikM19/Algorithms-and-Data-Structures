...
public class Main {
    public static void main(String[] args) {
        ...
        int removeNetwork = sc.nextInt();
        graph.removeVertex(removeNetwork);

        for (int i = 0; i < list.size(); i++) {
            if (list.get(i) == removeNetwork) {
                list.remove(i);
            }
        }
        ...
        for (int i = 0; i < list.size(); i++) {
            if (!map.contains(list.get(i))) {
                count++;
                checkGroups(map, list.get(i), graph);
            }
        }
    ...
    private static void checkGroups(Set<Integer> map, Integer integer, AdjacencyListGraph<Integer> graph) {
        ...
        for (int network: graph.getNeighbors(integer)) {
            checkGroups(map, network, graph);
        }
    }
